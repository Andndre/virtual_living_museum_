    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="phone_number" name="phone_number" type="text"
                class="{{ !auth()->user()->phone_number ? 'border-yellow-300 bg-yellow-50' : '' }} mt-1 block w-full"
        ...
        <div>
            ...
            <x-text-input id="address" name="address" type="text"
                class="{{ !auth()->user()->address ? 'border-yellow-300 bg-yellow-50' : '' }} mt-1 block w-full"
        ...
        <div>
            ...
            <div class="relative">
                <x-text-input id="date_of_birth" name="date_of_birth" type="text"
                    class="date-picker {{ !auth()->user()->date_of_birth ? 'border-yellow-300 bg-yellow-50' : '' }} mt-1 block w-full"
                    :value="old('date_of_birth', $user->date_of_birth ? $user->date_of_birth->format('Y-m-d') : '')" autocomplete="off" placeholder="YYYY-MM-DD" readonly onfocus="this.blur()" />
        ...
        <div>
            ...
            <x-text-input id="pekerjaan" name="pekerjaan" type="text"
                class="{{ !auth()->user()->pekerjaan ? 'border-yellow-300 bg-yellow-50' : '' }} mt-1 block w-full"
        ...
        <div class="flex items-center gap-4">
            <x-primary-button class="flex w-full justify-center">{{ __('app.save') }}</x-primary-button>

            @if (session('status') === 'profile-updated')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
                    class="text-sm text-gray-600">{{ __('app.saved') }}</p>
            @endif
        </div>
    </form>
</section>
