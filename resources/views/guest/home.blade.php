<x-guest-layout>
	...
	<div class="px-6 py-6 bg-primary text-white">
		<div class="flex justify-between items-center">
			...
			<a href="{{ route('profile.edit') }}" class="w-12 h-12 rounded-full overflow-hidden border-2 border-white/30 hover:border-white/50 transition-colors relative">
	...
	<div class="px-6 py-8 pb-36 bg-gray-50 rounded-t-3xl -mt-6 relative">
		<div class="grid grid-cols-3 gap-6">
			...
			{{-- Maps --}}
			<div class="flex flex-col items-center">
				<a href="{{ route('guest.maps') }}" class="flex items-center justify-center w-20 h-20 bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow mb-3">
			...
			{{-- Kritik & Saran --}}
			<div class="flex flex-col items-center">
				<a href="{{ route('guest.kritik-saran') }}" class="flex items-center justify-center w-20 h-20 bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow mb-3">
		...
		<div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
			...
			<div x-show="show"
				...
				<div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
					<a href="{{ route('profile.edit') }}" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-primary text-base font-medium text-white hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:ml-3 sm:w-auto sm:text-sm">
						Lengkapi Sekarang
					</a>
					<button type="button" @click="show = false; localStorage.setItem('profile_reminder_dismissed', '{{ date('Y-m-d') }}')" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
						Ingatkan Nanti
					</button>
				</div>
			</div>
