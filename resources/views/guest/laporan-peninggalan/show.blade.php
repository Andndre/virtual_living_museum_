<x-guest-layout>
    ...
    <div class="px-6 pt-6 pb-24 bg-gray-50 min-h-screen">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
            ...
            <div class="flex items-start mb-5">
                <div class="flex-shrink-0 mr-3">
                    @if($laporan->user->profile_photo)
                        <img src="{{ asset('storage/' . $laporan->user->profile_photo) }}" alt="User" class="w-12 h-12 rounded-full object-cover border border-gray-200">
                    @else
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm text-gray-500">{{ $laporan->created_at->format('d M Y, H:i') }} ({{ $laporan->created_at->diffForHumans() }})</p>
                </div>
            </div>
                ...
                <div class="mb-6">
                    <div class="gallery-carousel relative rounded-lg overflow-hidden">
                        ...
                        @if($laporan->laporanGambar->count() > 1)
                            ...
                            <button class="gallery-next absolute right-0 top-1/2 -translate-y-1/2 bg-black/50 text-white w-10 h-10 rounded-full flex items-center justify-center">
                                <i class="fas fa-chevron-right"></i>
                            </button>
                            <div class="gallery-dots absolute bottom-2 left-0 right-0 flex justify-center space-x-1">
                                @foreach($laporan->laporanGambar as $index => $image)
                                    <button class="gallery-dot w-2 h-2 rounded-full {{ $index === 0 ? 'bg-white' : 'bg-white/50' }}"></button>
                                @endforeach
                            </div>
                        @endif
                    </div>
            ...
            {{-- Description --}}
            <div class="mb-6">
                <h3 class="text-gray-700 font-medium mb-2">Deskripsi</h3>
                <div class="bg-gray-50 p-3 rounded-lg">
            ...
            <div class="flex justify-between border-t border-gray-100 pt-4">
                <div class="flex space-x-4">
                    <form id="like-form" action="{{ route('guest.laporan-peninggalan.like', $laporan->laporan_id) }}" method="POST">
                        @csrf
                    ...
                    <button id="comment-button" class="flex items-center space-x-1 text-gray-700 hover:text-primary transition-colors">
                ...
                <div class="flex space-x-3">
                    <div class="flex-shrink-0">
                        @if(auth()->user()->profile_photo)
                            <img src="{{ asset('storage/' . auth()->user()->profile_photo) }}" alt="User" class="w-8 h-8 rounded-full object-cover">
                        @else
                    ...
                    <div class="flex-1">
                        ...
                        @enderror
                        <button type="submit" class="mt-2 px-4 py-1 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors text-sm">
                            Kirim
                        </button>
                    </div>
                </div>
                    ...
                    <div class="flex space-x-3">
                        <div class="flex-shrink-0">
                            @if($comment->user->profile_photo)
                                <img src="{{ asset('storage/' . $comment->user->profile_photo) }}" alt="User" class="w-8 h-8 rounded-full object-cover">
                            @else
                                <div class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center">
                                    <i class="fas fa-user text-gray-500"></i>
                                </div>
