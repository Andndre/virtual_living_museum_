            ...
            <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between pt-2 border-t border-gray-100">
                            <div class="flex space-x-6">
                                <form class="like-form" data-id="{{ $item->laporan_id }}"
                                    action="{{ route('guest.laporan-peninggalan.like', $item->laporan_id) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="flex items-center space-x-1 text-gray-700 hover:text-primary transition-colors">
                                        <i
                                            class="like-icon {{ $item->laporanSuka->where('user_id', auth()->id())->count() > 0 ? 'fas' : 'far' }} fa-thumbs-up"></i>
                ...
                @empty
                    <div class="rounded-2xl bg-white p-12 text-center shadow-sm">
                        ...
                        <h3 class="mb-2 text-lg font-medium text-gray-900">{{ __('laporan-peninggalan.no_heritage_yet') }}
                        </h3>
