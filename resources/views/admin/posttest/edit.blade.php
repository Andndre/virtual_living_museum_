    ...
    <div class="py-6">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                ...
                <div class="mb-4">
                    <h1 class="text-2xl font-bold text-gray-900 sm:text-3xl">Edit Soal Posttest</h1>
                    <p class="mt-2 text-sm text-gray-600 sm:text-base">Mengedit soal posttest untuk materi:
                        <strong>{{ $posttest->materi->judul }}</strong></p>
                </div>

                <!-- Action Buttons -->
                <div class="flex flex-col gap-3 sm:flex-row sm:gap-3">
                    <a href="{{ route('admin.posttest.show', [$posttest->materi_id, $posttest->posttest_id]) }}"
            ...
            <div class="bg-white shadow sm:rounded-lg">
                <form action="{{ route('admin.posttest.update', [$posttest->materi_id, $posttest->posttest_id]) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                    <div class="space-y-6 px-4 py-5 sm:p-6">
                        ...
                        <div>
                            ...
                            <textarea name="pertanyaan" id="pertanyaan" rows="4"
                                ...
                                placeholder="Masukkan pertanyaan posttest" required>{{ old('pertanyaan', $posttest->pertanyaan) }}</textarea>
                            @error('pertanyaan')
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="text" name="pilihan_e" id="pilihan_e"
                                    value="{{ old('pilihan_e', $posttest->pilihan_e) }}"
                                    class="w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500"
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-3 md:grid-cols-5">
                                <label
                                    class="{{ old('jawaban_benar', $posttest->jawaban_benar) === 'A' ? 'bg-green-50 border-green-300' : '' }} flex cursor-pointer items-center rounded-md border border-gray-300 p-3 transition-colors hover:bg-gray-50">
                                    <input type="radio" name="jawaban_benar" value="A"
                                        class="mr-3 text-blue-600"
                                        {{ old('jawaban_benar', $posttest->jawaban_benar) === 'A' ? 'checked' : '' }}
                                        required>
                                    <span class="text-sm font-medium">Pilihan A</span>
                                </label>
                                <label
                                    class="{{ old('jawaban_benar', $posttest->jawaban_benar) === 'B' ? 'bg-green-50 border-green-300' : '' }} flex cursor-pointer items-center rounded-md border border-gray-300 p-3 transition-colors hover:bg-gray-50">
                                    <input type="radio" name="jawaban_benar" value="B"
                                        class="mr-3 text-blue-600"
                                        {{ old('jawaban_benar', $posttest->jawaban_benar) === 'B' ? 'checked' : '' }}
                                        required>
                                    <span class="text-sm font-medium">Pilihan B</span>
                                </label>
                                <label
                                    class="{{ old('jawaban_benar', $posttest->jawaban_benar) === 'C' ? 'bg-green-50 border-green-300' : '' }} flex cursor-pointer items-center rounded-md border border-gray-300 p-3 transition-colors hover:bg-gray-50">
                                    <input type="radio" name="jawaban_benar" value="C"
                                        class="mr-3 text-blue-600"
                                        {{ old('jawaban_benar', $posttest->jawaban_benar) === 'C' ? 'checked' : '' }}
                                        required>
                                    <span class="text-sm font-medium">Pilihan C</span>
                                </label>
                                <label
                                    class="{{ old('jawaban_benar', $posttest->jawaban_benar) === 'D' ? 'bg-green-50 border-green-300' : '' }} flex cursor-pointer items-center rounded-md border border-gray-300 p-3 transition-colors hover:bg-gray-50">
                                    <input type="radio" name="jawaban_benar" value="D"
                                        class="mr-3 text-blue-600"
                                        {{ old('jawaban_benar', $posttest->jawaban_benar) === 'D' ? 'checked' : '' }}
                                        required>
                                    <span class="text-sm font-medium">Pilihan D</span>
                                </label>
                                <label
                                    class="{{ old('jawaban_benar', $posttest->jawaban_benar) === 'E' ? 'bg-green-50 border-green-300' : '' }} flex cursor-pointer items-center rounded-md border border-gray-300 p-3 transition-colors hover:bg-gray-50">
                                    <input type="radio" name="jawaban_benar" value="E"
                                        class="mr-3 text-blue-600"
                                        {{ old('jawaban_benar', $posttest->jawaban_benar) === 'E' ? 'checked' : '' }}
                                        required>
                                ...
                                <div class="ml-3">
                                    ...
                                    <div class="mt-2 text-sm text-blue-700">
                                        ...
                                        @if ($posttest->updated_at != $posttest->created_at)
                                            <p>Terakhir diperbarui: {{ $posttest->updated_at->format('d M Y H:i') }}
                                            </p>
                                        @endif
                                    </div>
                                </div>
