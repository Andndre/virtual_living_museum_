            ...
            <div class="mt-8">
                <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                    <div class="px-4 py-5 sm:p-6">
                        <form action="{{ route('admin.riwayat-pengembang.update', $riwayatPengembang->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="space-y-6">
                                <!-- Judul -->
                                <div>
                                    ...
                                    <div class="mt-1">
                                        <input type="text" name="judul" id="judul" required
                                               ...
                                               value="{{ old('judul', $riwayatPengembang->judul) }}">
                                        @error('judul')
                                ...
                                <div>
                                    ...
                                    <div class="mt-1">
                                        <input type="date" name="tahun" id="tahun" required
                                               class="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md"
                                               value="{{ old('tahun', \Carbon\Carbon::parse($riwayatPengembang->tahun)->format('Y-m-d')) }}">
                                        @error('tahun')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div class="mt-1">
                                        <input type="date" name="tahun_selesai" id="tahun_selesai"
                                               class="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md"
                                               value="{{ old('tahun_selesai', $riwayatPengembang->tahun_selesai ? \Carbon\Carbon::parse($riwayatPengembang->tahun_selesai)->format('Y-m-d') : '') }}">
